...
export default function FundamentacionTab({ fundamentacion, planificacionId }: FundamentacionTabProps) {
  ...
  return (
    <article
      className="rounded-2xl bg-[#F1F6F2] border-l-4 border-l-green-primary p-5 sm:p-6"
      aria-label="Fundamentación pedagógica"
    >
      <div className="flex items-start gap-3.5 mb-5">
        <span className="shrink-0 flex items-center justify-center w-11 h-11 rounded-full bg-green-primary text-white">
          <BookIcon className="w-[22px] h-[22px]" />
        </span>
        <div>
          <h3 className="text-xl font-semibold font-quicksand text-green-dark">
      ...
      {planificacionId ? (
        <EditableBlock
          ...
          className="text-[15px] font-quicksand text-text-dark leading-relaxed"
          as="div"
          pencilSiempreVisible
        />
      ) : (
        <div className="text-[15px] font-quicksand text-text-dark leading-relaxed whitespace-pre-line">
          {fundamentacion}
        </div>
      )}
    </article>
